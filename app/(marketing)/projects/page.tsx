      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-20">
          {stats.map((stat, idx) => (
            <div
              key={idx}
              className="glass-effect rounded-xl border border-yellow-500/30 p-6 text-center hover:border-yellow-400/60 transition-all"
            >
        ...
        <div className="space-y-12 mb-20">
          {projects.map((project, idx) => (
            <div
              key={project.id}
              className={`glass-effect rounded-2xl border border-yellow-500/30 overflow-hidden hover:border-yellow-400/60 transition-all ${
            ...
            >
              ...
              <div className={`p-8 lg:col-span-3 ${idx % 2 === 1 ? "lg:order-2" : ""}`}>
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="inline-block bg-yellow-500/20 text-yellow-300 px-3 py-1 rounded-full text-sm font-medium mb-3">
                      {project.industry}
                    </div>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-gray-400 text-sm mt-1">ROI Achieved</p>
                  </div>
                </div>

                {/* Challenge */}
                <div className="mb-6">
                  <h3 className="text-yellow-400 font-semibold mb-2">The Challenge</h3>
                ...
                {/* Results */}
                <div className="mb-8">
                  <h3 className="text-yellow-400 font-semibold mb-3">Results & Impact</h3>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.technologies.map((tech, i) => (
                    <span
                      key={i}
                      className="inline-block bg-yellow-500/10 border border-yellow-500/30 text-yellow-300 px-3 py-1 rounded-full text-sm"
                    >
                      {tech}
                    </span>
                  ))}
              ...
              {/* Testimonial */}
              <div
                className={`p-8 bg-gradient-to-br from-yellow-500/10 to-transparent border-t lg:border-t-0 lg:col-span-2 flex flex-col justify-between ${
                  idx % 2 === 1 ? "lg:order-1" : ""
                }`}
              >
                <div>
                  ...
                  <blockquote className="text-white text-lg mb-6 leading-relaxed italic">
                    "{project.testimonial.text}"
                  </blockquote>
                </div>
                <div>
                  <p className="font-semibold text-white">{project.testimonial.author}</p>
                  <p className="text-yellow-300 text-sm">{project.testimonial.role}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        {/* CTA Section */}
        <div className="glass-effect rounded-2xl border border-yellow-500/30 p-12 text-center bg-gradient-to-r from-yellow-500/10 to-transparent">
